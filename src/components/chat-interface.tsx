...
export function ChatInterface() {
  ...
  return (
    <Card className="flex h-full max-h-105 flex-col border-white/15 bg-slate-950/40 p-3 backdrop-blur-2xl">
      <div className="flex items-center justify-between gap-3 rounded-2xl border border-white/10 bg-slate-900/80 px-3 py-2 text-sm text-slate-200 shadow-sm">
      ...
      <div className="mt-3 flex-1 max-h-105 rounded-2xl border border-white/10 bg-slate-950/60 px-3 py-2">
        <div className="flex h-full flex-col gap-2 overflow-y-auto pb-1 pt-0">
          {showWelcome && (
            <div className={"mt-4 space-y-2 rounded-2xl bg-slate-900/80 p-3 text-sm text-slate-300 ring-1 ring-white/10"}>
          ...
          {completion && (
            <div className="flex justify-start">
              <div className="inline-flex max-w-[85%] items-start gap-2 rounded-2xl bg-slate-900/80 px-3 py-2 text-sm text-slate-50 shadow-sm ring-1 ring-white/10">
                <div className="mt-px">
      ...
      <form
        ...
        className="mt-3 flex items-center gap-2 rounded-2xl border border-white/15 bg-slate-950/70 p-2 text-sm shadow-inner shadow-slate-950/60"
      >
        ...
        <Input
          ...
          className="h-9 border-none bg-transparent text-sm text-slate-100 placeholder:text-slate-500 focus-visible:ring-0 focus-visible:ring-offset-0"
        />
        <Button
          type="submit"
          size="icon-sm"
        ...
        <div className="mt-3 flex flex-wrap gap-2 px-1">
          {sources.map((source, index) => (
            <button
              key={source.id ?? index}
              type="button"
              className="rounded-full border border-white/15 bg-slate-900/80 px-2.5 py-1 text-sm text-slate-200 shadow-sm backdrop-blur hover:border-sky-400 hover:text-sky-100"
              onClick={() => {
